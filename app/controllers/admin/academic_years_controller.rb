class Admin::AcademicYearsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @academic_year.save
        format.html { redirect_to [:admin, @academic_year], notice: 'Academic year was successfully created.' }
        format.json { render :show, status: :created, location: [:admin, @academic_year] }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_academic_years_path, notice: 'Academic year was successfully destroyed.' }
