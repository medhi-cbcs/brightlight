namespace :data do
	desc "Check import student distribution"
	task check_import_student_dist: :environment do
    ...
    sheet.each_with_index(header) do |row,i|
      next if i < 1
      # next if row[:academic_year] != '2015-2016' || row[:grade_name] != 'PS-A'
      next if row[:grade_name] != 'PS-A'
      # break if i > 20
