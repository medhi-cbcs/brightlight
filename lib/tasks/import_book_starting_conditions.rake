namespace :data do
	...
	task import_book_starting_conditions: :environment do
    ...
    conditions = ['new','good','fair','poor','missing'].map {|condition| BookCondition.find_by_slug(condition)}
		...
		if use_sql_server
			...
			results = client.execute('SELECT * FROM CBCS_INVWRITECONDITIONS')
			...
			results.each_with_index do |row, i|
				...
				if copy.present?
					data = [copy.id, barcode, conditions[row[header[:condition_id]].to_i - 1].id, row[header[:date_input]], year_ids[row[header[:new_acad_year]]], row[header[:notes]], 0]
				else
					data = [nil, barcode, conditions[row[header[:condition_id]].to_i - 1].id, row[header[:date_input]], year_ids[row[header[:new_acad_year]]], barcode, 0]
				end
				values << data
	    ...
	    sheet = xl.sheet('CBCS_INVWRITECONDITIONS')
