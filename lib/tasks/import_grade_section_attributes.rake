namespace :data do
    ...
    parallel_codes = ['CG001.1', 'CG001.2', 'CG001.3', 'CG002.1', 'CG002.2', 'CG002.3', 'CG003.1', 'CG003.2', 'CG003.3', 'CG004.1', 'CG004.2', 'CG004.3', 'CG005.1', 
								      ...
								      'CG009.3', 'CG010.1', 'CG010.2', 'CG011.1', 'CG011.1', 'CG012.1', 'CG012.1', 'CG013.1', 'CG013.1', 'CG013.2', 'CG013.2', 'CG014.1', 'CG014.1', 
									    'CG014.2', 'CG014.2', 'CG014.3', 'CG014.3', 'CG015.1', 'CG015.1', 'CG015.2', 'CG015.2', 'CG015.3', 'CG015.3']
    ...
    header = { grade_id:"ClassIDGrade", employee_no:"ClassIDEmployee",name:"ClassEmployeeName", position:"ClassStatusCrew",
		...
		sheet.each_with_index(header) do |row,i|
		  ...
		  grade_section = GradeSection.find_by_name(section_names[parallel_codes.index(parallel_code)])

		  if row[:academic_year] != "2015-2016"
			  grade_section.subject_code = row[:subject_code]
			  ...
			  grade_section.save
				...
				year = AcademicYear.find_by_slug(academic_year)
				grade_section_history = grade_section.grade_section_histories.find_by_academic_year_id(year.id)
			  grade_section_history.subject_code = row[:subject_code]
			  grade_section_history.parallel_code = row[:parallel_code]
			  grade_section_history.homeroom = Employee.find_by_employee_number(row[:employee_no]) if row[:position] == '001'
			  grade_section_history.assistant = Employee.find_by_employee_number(row[:employee_no]) if row[:position] == '003'
			  grade_section_history.save				
  		end

  		puts "#{i}. #{row[:parallel_code]} #{row[:academic_year]}"
