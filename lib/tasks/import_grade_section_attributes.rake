namespace :data do
    ...
    header = { grade_id:"ClassIDGrade", employee_no:"ClassIDEmployee",name:"ClassEmployeeName", position:"ClassStatusCrew",
		...
		sheet.each_with_index(header) do |row,i|
		  ...
		  if subject_classes.include? row[:subject_code]
				section_name = section_names[subject_codes.index(row[:subject_code])]
			else
				section_name = section_names[parallel_codes.index(row[:parallel_code])]
			end
